...
TODO:
- Document how $(..).unbind('.editInPlace') can be used to remove an inline editor (that is closed) and how to deal with open ones.
  Consider adding $(..).editInPlace.remove() or something similar. For API ideas, look at the jquery ui plugins
...
- Check if the editor reset should happen before / after the new content has been set (its currently before, and there are some failing tests when this is changed, it seems logical though to do it the other way around)
...
$.fn.editInPlace = function(options) {
	...
	return this.each(function() {
		var dom = $(this);
        settings = $.extend({},settings,dom.metadata());
...
$.fn.editInPlace.defaults = {
	...
	textarea_rows:		20, // integer: set rows attribute of textarea, if field_type is set to textarea
	textarea_cols:		100, // integer: set cols attribute of textarea, if field_type is set to textarea
...
// REFACT: consider to remove exposure of class as it is not needed outside
...
$.extend(InlineEditor.prototype, {
	...
	workaroundFirefoxBlurBug: function() {
		...
		// Workaround for the tests where the editor is not a child of window.document but of a document fragment
	...
	handleSaveEditor: function() {
        ...
        var displayText = this.settings.field_type == "select" ?
                          this.dom.find('option:selected').text() : enteredText;
		...
		if (this.settings.callback)
			this.handleSubmitToCallback(enteredText,displayText);
		else
			this.handleSubmitToServer(enteredText,displayText);
	},
	...
	handleSubmitToCallback: function(enteredText,displayText) {
		...
		var newHTML = this.settings.callback(this.id(), enteredText, this.originalHTML,
	...
	handleSubmitToServer: function(enteredText,displayText) {
		...
		$.ajax({
			...
			success: function(html){
                ...
                var new_text = that.settings.ignore_server_response ? displayText : (html || that.settings.default_text);
