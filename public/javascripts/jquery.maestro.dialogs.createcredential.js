(function($){
    ...
    $.maestro.dialogs.createCredential = function(options)
    {
        options = options || {};

        var dlg_id = "maestro_dialogs_createCredential";
        var dlg = $("#" + dlg_id);
        if (dlg.length == 0){
            dlg = $("<div id='" + dlg_id + "'></div>");
            dlg.load("/admin/credentials/new",function(){
        ...
        function showDialog(dlg,options)
        {
            ...
            options.title = options.title || "Create a new credential";
