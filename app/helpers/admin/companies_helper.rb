module Admin::CompaniesHelper
  ...
  def org_subtree_html(tree_hash)
    ...
    tree_hash.keys.each do |node|
      output << "<li id='#{node.id}' class='jstree-open'' rel='organizational-unit' data-type='#{node.class.name.downcase}' data-collection-name='#{node.class.name.pluralize.underscore}'><a href='#'>#{node.name}</a>"
